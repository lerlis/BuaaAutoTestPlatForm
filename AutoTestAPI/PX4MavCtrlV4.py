...
class PX4MavCtrler:
    ...
    def __init__(self, test_id, port=20100,ip='127.0.0.1'):
        ...
        self.uavAngQuatern = [0, 0, 0, 0] # Estimated simulated AngQuatern from PX4 in NED frame  
        ...
        # fault inject correlation
        self.Test_ID = test_id
        self.flight_status_param = [0, 0]

        self.f_index = 0
        self.f_id = 0
        self.f_mode = 0
        self.f_controls = [0]*16
        self.f_pg_time = 0
        self.f_sleep_status = 0
        self.f_flight_status = 0
    ...
    def EndTrueDataLoop(self):
        ...
        self.tPX4.join(2) # 如果不加参数程序会一直卡住在这里
    ...
    def getBasicRbe(self):
        while(True):
            time.sleep(0.01)
            # mav_pos in ENU
            mav_pos = np.array([self.uavPosNED[1], self.uavPosNED[0], -self.uavPosNED[2]])
            self.mav_pos = mav_pos - self.mav_pos_offset
            self.mav_height = self.mav_pos[2]
            # attitude in NED
            q0, q1, q2, q3 = self.uavAngQuatern[0], self.uavAngQuatern[1], self.uavAngQuatern[2], self.uavAngQuatern[3]
            self.mav_yaw_g = np.arctan2(2*(q0*q3 + q1*q2), 1-2*(q2*q2 + q3*q3)) # yaw about world frame

            self.mav_roll = np.arctan2(2*(q0*q1 + q2*q3), 1-2*(q1*q1 + q2*q2))
            self.mav_pitch = np.arcsin(2*(q0*q2 - q3*q1))
            # rotation matrix of body2earth
            self.mav_Rbe_0 = eulerAngleToMatrix([self.mav_roll,self.mav_pitch,self.mav_yaw_g])
            # transfer attitude to ENU and FLU, get the Rbe
            # Attention: Rbe_0 is in FRD and NED
            self.R_NED2ENU = np.array([
                [0,1,0],
                [1,0,0],
                [0,0,-1]
            ])
            self.R_FLU2FRD = np.array([
                [1,0,0],
                [0,-1,0],
                [0,0,-1]
            ])
            self.mav_Rbe_1 = self.R_NED2ENU.dot(self.mav_Rbe_0)
            self.mav_Rbe = self.mav_Rbe_1.dot(self.R_FLU2FRD)
            self.mav_yaw_g = -self.mav_yaw_g + np.pi/2
            self.mav_yaw = constrain_rad(self.mav_yaw_g - self.mav_yaw_offset) # yaw about takeoff state
    ...
    def InitMavLoop(self,UDPMode=2):
        ...
        self.t3 = threading.Thread(target=self.getBasicRbe, args=())
        self.t3.start()
        self.startTime = time.time()
        self.lastTime2 = 0
    ...
    def sendMavFaultInfoAPI(self,index=0,f_id=0,mode=0,controls=[0]*16,pg_time=0,sleep_status=0,flight_status=0):
        """send fault injection information to PX4, the definition of the mavlink message is same as
        that in real flight experiment by LXL. The docs can be found at : 
        """
        if self.isInPointMode:
            return
        time_boot_ms = int((time.time()-self.startTime)*1000)
        if self.isCom:
            self.the_connection.mav.hil_actuator_controls_send(int(time_boot_ms),controls,int(mode),int(f_id))
            # self.the_connection.mav.rfly_ctrl_lxl_send(int(time_boot_ms),int(index),int(f_id),int(mode),controls)
            time.sleep(0.1)
            self.the_connection.mav.hil_controls_send(int(time_boot_ms),pg_time,0,0,0,0,0,0,0,int(sleep_status),int(flight_status))
        else:
            if self.UDPMode>1.5:
                buf = self.mav0.hil_actuator_controls_encode(int(time_boot_ms),controls,int(mode),int(f_id)).pack(self.mav0)
                # buf = self.mav0.rfly_ctrl_lxl_encode(int(time_boot_ms),int(index),int(f_id),int(mode),controls).pack(self.mav0)
                self.udp_socket.sendto(buf, (self.ip, self.port))
                time.sleep(0.1)
                buf1 = self.mav0.hil_controls_encode(int(time_boot_ms),pg_time,0,0,0,0,0,0,0,int(sleep_status),int(flight_status)).pack(self.mav0)
                self.udp_socket.sendto(buf1, (self.ip, self.port))
            else:
                # neglect UDPMode < 1.5 now, only suitable for UDPMode = 2
                pass
    ...
    def SendVelFLU(self,vx=0,vy=0,vz=0,yawrate=0):
        """ Send vehicle targe speed (m/s) to PX4 in the body forward-leftward-upward (FLU) frame with yawrate control (rad/s)
        when the vehicle fly upward, the vz > 0
        """
        self.EnList = [0,1,0,0,0,1]
        self.type_mask=self.TypeMask(self.EnList)
    ...
    def SendFaultInformation(self, f_index, f_id, f_mode, f_controls):
        self.f_index = f_index
        self.f_id = f_id
        self.f_mode = f_mode
        self.f_controls = f_controls
    
    def SendOffbInformation(self, f_pg_time, f_sleep_status, f_flight_status):
        self.f_pg_time = f_pg_time
        self.f_sleep_status = f_sleep_status
        self.f_flight_status = f_flight_status

    # def start_injection_pub(self):
    #     self.fault_injection_th = threading.Thread(target=self.fault_injection_pub_loop, args=())
    #     self.fault_injection_th.start()
    #     time.sleep(1)
    #     print("start pub fault infromation")

    # def fault_injection_pub_loop(self):
    #     pub_time = time.time()
    #     while (self.judge_flag):
    #         if (time.time() - pub_time) > 1/50.0:
    #             pub_time = time.time()
    #             self.SendFaultInformation(self.command_fault[0], self.command_fault[1], self.command_fault[2], self.command_fault[3])
    #         time.sleep(0.02)       
    #     self.stop_thread(self.fault_injection_th.ident)
    ...
    # def start_flight_mode_pub(self):
    #     self.offb_info_th = threading.Thread(target=self.offb_info_pub_loop, args=())
    #     self.offb_info_th.start()
    #     time.sleep(1)
    #     print("start pub flight mode infromation") 

    # def offb_info_pub_loop(self):
    #     pub_time = time.time()
    #     while (self.judge_flag):
    #         if (time.time() - pub_time) > 1/50.0:
    #             pub_time = time.time()
    #             self.SendOffbInformation(self.command_flight_mode[0], self.command_flight_mode[1], self.command_flight_mode[2])
    #         time.sleep(0.02)    
    #     self.stop_thread(self.offb_info_th.ident)
    ...
    # send target position in earth ENU frame
    def SendPosENU(self,x=0,y=0,z=0,yaw=0):
        """ Send vehicle targe position (m) to PX4 in the earth east-north-up (ENU) frame with yaw control (rad)
        when the vehicle fly above the ground, then z > 0
        """
        self.EnList = [1,0,0,0,1,0]
        self.type_mask=self.TypeMask(self.EnList)
    ...
    def initOffboard(self):
        ...
        if self.UDPMode>1.5:
            ...
            self.sendMavOffboardAPI(self.type_mask, self.coordinate_frame, self.pos, self.vel, self.acc, self.yaw,
                                    self.yawrate)
            time.sleep(0.1)
            self.sendMavFaultInfoAPI(self.f_index, self.f_id, self.f_mode, self.f_controls, self.f_pg_time, self.f_sleep_status, self.f_flight_status)
            time.sleep(0.1)
            self.sendMavOffboardAPI(self.type_mask, self.coordinate_frame, self.pos, self.vel, self.acc, self.yaw,
                                    self.yawrate)
            time.sleep(0.1)
            ...
            while True:
                ...
                if self.UDPMode>1.5: # If Use MAVLink Mode
                    msg = self.the_connection.recv_match(
                        type=['ATTITUDE', 'LOCAL_POSITION_NED',"ATTITUDE_QUATERNION",'HEARTBEAT','HOME_POSITION','GLOBAL_POSITION_INT','HIL_ACTUATOR_CONTROLS','STATUSTEXT'],
                        blocking=False)
                    if msg is not None:
